...
async function get(page, perPage, stratregy) {
    try {
        var response = await fetch(`${URL_BASE}/contents?page=${page}&per_page=${perPage}&strategy=${stratregy}`);
...
function createFooter(content) {
    let divFooter = document.createElement('div');
        ...
        divFooter.appendChild(createFooterSpan(`${content.children_deep_count} comentários`));
